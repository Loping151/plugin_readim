...
现在。自己要求的读图只在当前会话中，即群间不共享，但同一群同时的用户间共享，因为我没找到插件基类传入的context里有精确到用户，@用户过程是在外部添加的。OCR状态也更改为当前会话有效，且可以设置默认是否有效。对比0.1版本中，发起一次请求，在别的群或私聊中，别人也能用，且不同群发起会冲突。OCR状态为全局。
